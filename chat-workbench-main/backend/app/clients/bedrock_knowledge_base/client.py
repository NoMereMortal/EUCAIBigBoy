...
class BedrockKnowledgeBaseClient(BaseClient):
    """Bedrock Knowledge Base client with async operations."""
    ...
    async def cleanup(self) -> None:
        """Cleanup Bedrock Knowledge Base client."""
        if self._client:
            with self.monitor_operation(get_function_name()):
                await self._client.__aexit__(None, None, None)
                logger.info('Bedrock Knowledge Base client closed')
    ...
    async def get_sync_client(self) -> Any:
        ...
        if not self._sync_client:
            with self.monitor_operation(get_function_name()):
                ...
                logger.info('Bedrock Knowledge Base sync client initialized')
